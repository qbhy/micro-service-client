...
use Qbhy\MicroServiceClient\UserCenter\JwtParser\Parser;
...
class UserCenterGuard implements Guard
{
    use GuardHelpers;
    ...
    public function user()
    {
        ...
        if ($token && ($payload = $this->jwt->fromToken($token)->getPayload())) {
            $payload = [
                'iuid' => unserializeUid($payload['iuid']),
                'guid' => isset($payload['guid']) ? unserializeUid($payload['guid']) : null,
            ];
     ...
     * @return string
     */
    public function login(UserCenterSubject $user)
    {
        $this->user = $user;
        return true;
    }
